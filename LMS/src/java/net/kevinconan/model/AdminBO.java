...
public class AdminBO {
	...
	public boolean checkAdmin(String u, String p) {
		...
		b = new SimpleMessageDigest().isStringMatchSHA1(p, pwdb);
	...
	public boolean createAdmin(String u, String p) {
		...
		try {
			...
			String pws = new SimpleMessageDigest().getSHA1String(p);
	...
	public AdminBean getAdminById(String id) {
		...
		try {
			if (rs.next()) {
				ab = new AdminBean();
				ab.setADID(rs.getString(1));
				ab.setAPASSWD(rs.getString(2));
				ab.setANAME(rs.getString(3));
				ab.setAGENDER(rs.getString(4));
				ab.setAVATAR(rs.getString(5));
			}
		} catch (SQLException ex) {
			Logger.getLogger(AdminBO.class.getName()).log(Level.SEVERE, null, ex);
		}
