...
public class BookBO {
	...
	public boolean updateBook(BookBean bb) {
		boolean b = false;
		String sql = "update bookcard set ISBN='" + bb.getISBN() + "',BNAME='" + bb.getBNAME() + "',AUTHER='" + bb.getAUTHER() + "',PRESS='" + bb.getPRESS() + "',PTIME='" + bb.getPTIME() + "',BTYPE='" + bb.getBTYPE() + "',PRICE='" + bb.getPRICE() + "',BAUTH='" + bb.getBAUTH() + "',SNUM='" + bb.getSNUM() + "',BNUM='" + bb.getBNUM() + "'";
		ConnDB cdb = new ConnDB();
		cdb.connect();
		cdb.setSqlStatement(sql);
		cdb.execUpdate();
	...
	public ArrayList searchBook(int pageNow, int pageSize) {
		...
		String sql = "select * from BOOK WHERE CONCAT(AUTHER,PRESS,BNAME) LIKE '%" + searchPattern + "%' AND CONCAT(AUTHER,PRESS,BNAME) IS NOT NULL limit " + (pageNow - 1) * pageSize + "," + pageSize;
		ConnDB cdb = new ConnDB();
		cdb.connect();
		cdb.setSqlStatement(sql);
		cdb.execQuery();
